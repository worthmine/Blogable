...
| Code | Kind | Trigger |
|------|------|---------|
| E201 | Error | Invalid front matter key (not in allowed list, not `x-*`) |
| E202 | Error | Unknown MetaKey in a modifier `@[key: value]` |
| E401 | Error | Invalid list indentation (odd number of leading spaces) |
| E402 | Error | Heading level out of range (more than 6 colons; maximum is h6) |
| E403 | Error | Definition block has no body text (DD is required by spec) |
| W601 | Warning | Unresolved internal anchor reference `[#id]` |
| W201 | Warning | Malformed front matter line (does not match `key: value` format) |
| W202 | Warning | Unterminated front matter block (EOF reached without closing `@@`) |
| W001 | Warning | Unterminated code block (EOF reached without closing `!#`) |
| W002 | Warning | Unterminated quote block (EOF reached without closing `<|`) |
| W003 | Warning | Unterminated math block (EOF reached without closing `$$`) |
| W401 | Warning | Duplicate definition term (same term defined more than once) |
| W801 | Warning | Orphaned modifier (not consumed by any block) |
