...
function tokenize(lines) {
  ...
  if (mode==='front')   pushDiag('W204','Unterminated front matter: @@ was opened but the closing @@ was not found before EOF.');
...
function buildAST(tokens) {
  ...
  function parseFrontLines(lines) {
    ...
    for (const line of lines) {
      ...
      if (!m) {
        // [W203] Non-empty lines that do not match FrontMetaLine syntax are invalid
        if (line !== '') pushDiag('W203',`Malformed front matter line: "${line}". Expected format: "key: value".`);
        continue;
      }
